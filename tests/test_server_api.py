...
from unittest.mock import patch  # Hanya 'patch' yang dipakai
...
def test_get_status_area(
    client, area_id_param, clean_areas_data_fixture
):  # clean_areas_data_fixture untuk isolasi
    ...
    data = response.json  # Menggunakan .json dari Flask test response
    assert data["area"] == area_id_param
    assert data["connection_status"] is True
    assert data["has_frame"] is True


def test_get_combined_status_api(client, clean_areas_data_fixture):
...
def test_upload_image_area_success(
    mock_process_image, client, area_id_param, clean_areas_data_fixture
):
...
@pytest.mark.parametrize("area_id_param", ["A1", "A2"])
def test_get_detections_area_api(client, area_id_param, clean_areas_data_fixture):
    from neopark_server import areas_data
...
def test_get_combined_detections_api(client, clean_areas_data_fixture):
    from neopark_server import areas_data
...
def test_video_feed_area(
    mock_generate_frames, client, area_id_param, clean_areas_data_fixture
):
    ...
    mock_generate_frames.side_effect = (
        dummy_frame_generator  # Gunakan side_effect untuk generator
    )
