...
_mock_yolo_instance_for_tests_cache = (
    None  # Variabel global untuk menyimpan mock instance
)
...
def get_flask_app_with_mocks():
    ...
    if _flask_app_global_cache is None:
        ...
        # --- PERUBAHAN DI SINI ---
        # Ganti nama variabel yang tidak dipakai menjadi '_'
        with patch(
            "ultralytics.YOLO", return_value=mock_yolo_instance
        ) as _, patch(  # Variabel _mock_ultralytics_yolo diganti menjadi _
            "neopark_server.YOLO", return_value=mock_yolo_instance, create=True
        ) as _:  # Variabel _mock_neopark_server_yolo diganti menjadi _
            # --- AKHIR PERUBAHAN ---
            ...
            _flask_app_global_cache.mock_yolo_instance = (
                _mock_yolo_instance_for_tests_cache
            )
...
@pytest.fixture(
    autouse=True
)  # autouse=True agar fixture ini otomatis dipakai di setiap tes
...
def clean_areas_data_fixture(
    app_instance,
):
    from neopark_server import areas_data

    # Reset ke state awal yang diketahui untuk setiap area
    for area_id_key in list(
        areas_data.keys()
    ):  # list() untuk menghindari error size change saat iterasi jika ada modifikasi
